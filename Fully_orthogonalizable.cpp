...
int pos = l - 1;
vector<BinaryVector> cands;
for (auto &w : dual_codewords) if (w[pos].val == 1) cands.push_back(w);
sort(cands.begin(), cands.end());
cands.erase(unique(cands.begin(), cands.end()), cands.end());
vector<int> chosen, usage(l, 0);
...
string q1 = "Cau 1: Tong kiem tra truc giao nao la phu hop voi bai (tai vi tri e_" + to_string(l - 1) + ")?";
vector<pair<string, bool>> opts1;

if (found) {
    stringstream ss;
    ss << "Bo " << J << " phuong trinh (tu ma) truc giao gom:\n";
    for (int idx : chosen) {
        ss << "     "; 
        for(int bit = 0; bit < l; bit++) ss << cands[idx][bit].val;
        if (idx != chosen.back()) ss << "\n";
    }
    ...
    opts1 = {
        ...
        {"Tat ca cac tu ma doi ngau co bit 1 o vi tri e_" + to_string(pos) + ".", false},
        {"Bo " + to_string(J) + " vector duoc chon ngau nhien tu khong gian ma.", false},
        {"Bo " + to_string(J) + " vector toan so 1.", false}
    };
}

ask_question(q1, opts1, "");
